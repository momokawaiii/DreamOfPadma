...
- 英文原文：TASK-021-ABC-Sandbox-Operations.md
- 文档 ID：TASK-021
- 版本：0.1
- 状态：Backlog
...
排除：推导 ABC 战内合法性、完整升级树、任意召唤家族及绕过世界状态服务。 其他任务源代码／内容、上方未列共享配置、参考原型、生成目录和新独立 UE 模块均不在范围内。已有 Demo 代码通过公开契约消费；适配／迁移须有明确命名的写集。

## 依赖、交接与冲突

- 必要前置：[TASK-019](TASK-019-Resource-and-Global-Ledgers.zh-CN.md), [TASK-020](TASK-020-Card-Catalog-and-Lifecycle.zh-CN.md), [TASK-024](TASK-024-Sandbox-Graph-and-Movement.zh-CN.md)。
...
- [ ] 记录选定 A／B 例子所需的 B 场积累／时点／费用及移动／附着语义。
- [ ] 放置／附着／直接使用／部署命令校验卡身份、节点和费用；失败保持卡与世界状态不变。
- [ ] ABC 主动技能在配置限制下可从沙盒发动；基础非 A 战斗卡在沙盒被拒绝。
- [ ] C 角色与建筑创建关联权威 ID 的可见表示；事件和快照参与覆盖附着与部署状态。
- [ ] 精确写集、同步任务／模块文档、必要检查和独立评审通过；没有以默认值填补未决规则。

## 验证与证据

有效／无效放置、附着、付款及沙盒技能测试；用户操作 A／B／C 并观察无重复部署。

原生代码变化时编译 Editor 目标。对实际实现的狭窄自动化，使用 Scripts/RunTests.ps1，传入已验证 EngineRoot 和 TestFilter `DreamOfPadma.MVP.TASK021`；确认报告中的用例真正通过，而非只检查进程退出。运行 Scripts/AuditDocs.ps1、适用的 Scripts/ValidateProject.ps1 -Strict 及限定范围 git diff --check。不要仅为本任务建立大规模 UI 自动化。记录可复现用户 PIE 观察（书面证据可接受）、输入／配置 ID 和日志；图片／视频可补充。本契约不声称已观察运行或手工成功。

## 学习契约

| 目标 | 证据 | 状态 |
|---|---|---|
| 跨系统命令归属 | 追踪一次部署从卡牌／付款校验到节点变化和可见角色。 | Not started |
...
D29 已确定附着移动：A 与附着 B 成组，逐张检查移动政策。先从权威附着状态解析整组，再由 TASK-024 校验／提交；失败不能解绑或只移动／扣费一部分。附着／直接使用费用与时机仍独立待定。
