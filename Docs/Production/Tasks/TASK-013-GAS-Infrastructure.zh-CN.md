# TASK-013 原生 GAS 基础设施

- 英文原文：TASK-013-GAS-Infrastructure.md
- 文档 ID：TASK-013
- 版本：0.1
- 状态：Backlog
...
排除：可玩 ACT、跨模式统一技能生命周期、万能 AttributeSet、外部 GAS 插件及新 UE 模块。 其他任务源代码／内容、上方未列共享配置、参考原型、生成目录和新独立 UE 模块均不在范围内。已有 Demo 代码通过公开契约消费；适配／迁移须有明确命名的写集。
...
- [ ] 实现前记录已评审的 owner／avatar、初始化／重绑定及授予／移除策略；仅添加必要原生 GAS 依赖并编译。
- [ ] 各测试模式注册自身测试定义和发动适配器；错误模式定义失败，不回退到另一模式。
- [ ] 进入、退出、死亡及重建测试 avatar 后，不遗留可影响后续战斗实例的效果、回调、技能或输入授予。
- [ ] Core 公开契约和存档记录不含 GAS 运行句柄；工具函数不规定各模式的时钟、阶段、AttributeSet 或效果。
- [ ] 精确写集、同步任务／模块文档、必要检查和独立评审通过；没有以默认值填补未决规则。

## 验证与证据

Editor 编译、错误模式授予／发动拒绝及重复生命周期清理自动化；在小型 PIE 夹具检查独立模式注册。

原生代码变化时编译 Editor 目标。对实际实现的狭窄自动化，使用 Scripts/RunTests.ps1，传入已验证 EngineRoot 和 TestFilter `DreamOfPadma.MVP.TASK013`；确认报告中的用例真正通过，而非只检查进程退出。运行 Scripts/AuditDocs.ps1、适用的 Scripts/ValidateProject.ps1 -Strict 及限定范围 git diff --check。不要仅为本任务建立大规模 UI 自动化。记录可复现用户 PIE 观察（书面证据可接受）、输入／配置 ID 和日志；图片／视频可补充。本契约不声称已观察运行或手工成功。

## 学习契约

| 目标 | 证据 | 状态 |
|---|---|---|
| ASC 生命周期与模式隔离 | 预测已销毁战斗的回调能否影响下一场，再检查拒绝日志。 | Not started |
