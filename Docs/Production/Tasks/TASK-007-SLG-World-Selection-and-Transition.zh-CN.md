...
- 剩余风险和未决问题：Actor 初始化顺序已有编译、unattended smoke 和 PIE 日志证据，但没有专门的自动化生命周期回归测试；最终默认地图配置仍由 TASK-010 负责，并已在独立的本地提交 `d8f107f` 中存在，不属于 TASK-007；Encounter 场景仍由 TASK-008 负责；所有 Open/Proposed/Deferred 的世界、路线、ID 命名空间、导入、存档和 World Partition 决策保持不变。
