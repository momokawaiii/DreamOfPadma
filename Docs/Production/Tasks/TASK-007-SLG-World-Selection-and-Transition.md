...
- Review findings resolved or accepted: the final lit `DemoSandbox.umap` state is now included in local commit `f80f727` after the original implementation commit `859fe5a`; the World -> concrete Game/Session dependency was removed; World now exposes typed publisher/consumer callbacks and the GameInstance session subsystem binds them after map load. The initial invalid direct construction of `UGameInstanceSubsystem` remains resolved through map-free `FDemoTransitionContextStore` delegation. The changelog wording now limits retention to the same GameInstance/session. Independent implementation Review remains required before integration.
