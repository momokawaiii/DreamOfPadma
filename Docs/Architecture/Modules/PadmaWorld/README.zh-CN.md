...
TASK-007 会把一次性地块命中转换为 `FDemoNodeId`；Actor 指针和原始坐标不会进入 `FDemoTransitionContext`。World 暴露强类型切场发布/消费回调，PadmaGame 会话边界在地图中的 Actor 完成初始化后绑定这些回调，并拥有 `UDemoTransitionSessionSubsystem` 的发布/查看/消费操作。
...
在 TASK-007 中，World 只产生强类型选择和确认结果，不包含具体的 Game/会话子系统。PadmaGame 在地图中的 Actor 完成初始化后绑定 World 回调，并把有效上下文转交给会话存储；会话存储在替换 pending 上下文前校验三个身份，因此无效 fixture 会失败并保持已有上下文不变，同时保持 Game -> World 的组合方向。
